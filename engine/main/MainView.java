...
public class MainView extends View {
    ...
    public void onDraw(Canvas c) {
        ...
        Screen.screens.get(Camera.current_screen_id).repeat();   // display current screen

        if (touch_hold) {
            Screen.screens.get(Camera.current_screen_id).touchHold();    // trigger touch function in the right screen
        }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if (ev.getAction() == MotionEvent.ACTION_DOWN) {    // finger starts touching the screen
            touch_hold = true;
            Screen.screens.get(Camera.current_screen_id).touchDown();
        }

        if (ev.getAction() == MotionEvent.ACTION_UP) {  // finger loses touch
            touch_hold = false;
            Screen.screens.get(Camera.current_screen_id).touchUp();
        }
